...
ap.add_argument("-f", "--file", default="image_path",
    help="detection image path")
...
PATH_TO_CKPT = os.path.join("", 'DetectionModel/ssd_mobilenet_v1/inference_graph/frozen_inference_graph.pb')
...
# frame = cv2.imread('/Users/erdal/Developer/git/TER2021-074/Documentation/50-PANEL/20.jpg')
image = os.path.join(args["file"])
print(image)
...
if len(idxs) > 0:
    ...
    for i in idxs.flatten():
        ...
        if SCORE > 85:
            ...
            file_path = "/Users/erdal/Developer/git/TER2021-074/Documentation/Images_cropped"
            now = datetime.now()
            file_name = now.strftime("%H%M%S")
            im = (Image.fromarray(cropped_image,'RGB'))
            im.show()
            im.save(os.path.join(file_path)+file_name+".jpg")
